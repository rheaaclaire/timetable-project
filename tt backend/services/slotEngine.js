...
function buildParallelLabTasks(rawLabs) {
  const finalLabs = [];
  const groupedByDuration = {};

  rawLabs.forEach((lab) => {
    const duration = lab.labSlots || 2;

    if (!groupedByDuration[duration]) {
      groupedByDuration[duration] = [];
    }

    groupedByDuration[duration].push(lab);
  });

  Object.values(groupedByDuration).forEach((labs) => {
    for (let i = 0; i < labs.length; i += 3) {
      const group = labs.slice(i, i + 3);

      if (group.length < 3) {
        finalLabs.push(...group);
        continue;
      }

      for (let rotation = 0; rotation < 3; rotation += 1) {
        const batch1 = group[rotation % 3];
        const batch2 = group[(rotation + 1) % 3];
        const batch3 = group[(rotation + 2) % 3];

        const subjectName = [
          `Batch 1: ${batch1.subjectName}`,
          `Batch 2: ${batch2.subjectName}`,
          `Batch 3: ${batch3.subjectName}`
        ].join(" | ");
        ...
        finalLabs.push({
          subjectKey: `PARALLEL_LAB_${i}_ROT_${rotation}`,
          repeatGroupKey: `PARALLEL_LAB_${i}_ROT_${rotation}`,
          subjectName,
          faculty,
          type: "LAB",
          isLab: true,
          labSlots: group[0].labSlots,
          isMajorMinor: false,
          closesDay: false,
          enforceSingleWeekly: false
        });
      }
    }
  });
...
function buildBatchLabTasks(rawLabs) {
  const finalLabs = [];
  const grouped = {};
  ...
  rawLabs.forEach((lab) => {
    ...
    grouped[duration].push(lab);
  });

  Object.values(grouped).forEach((labs) => {
    for (let i = 0; i < labs.length; i += 3) {
      const group = labs.slice(i, i + 3);
      ...
      rotations.forEach((rotation, index) => {
        const batch1 = group[rotation[0]];
        const batch2 = group[rotation[1]];
        const batch3 = group[rotation[2]];

        finalLabs.push({
          subjectKey: `BATCH_GROUP_${i}_${index}`,
          repeatGroupKey: `BATCH_GROUP_${i}_${index}`,

          subjectName:
            `B1-${batch1.subjectName}\n` +
            `B2-${batch2.subjectName}\n` +
            `B3-${batch3.subjectName}`,
...
function buildTasks(subjects) {
  ...
  const labs = buildBatchLabTasks(rawLabs);
